...
export function StatCard({
  ...
  return (
    <motion.div
      ...
      className="glass-card rounded-2xl p-6 relative overflow-hidden group select-none transition-colors"
    >
      {/* Subtle specular corner reflection */}
      <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl from-white/[0.05] to-transparent pointer-events-none" />

      <div className="flex items-center justify-between">
        <p className="text-[11px] font-mono uppercase tracking-widest text-zinc-400 dark:text-zinc-500 font-semibold">
          {title}
        </p>
        <div className="w-8 h-8 rounded-xl bg-black/[0.04] dark:bg-white/[0.06] border border-black/5 dark:border-white/10 flex items-center justify-center text-zinc-800 dark:text-zinc-200 shrink-0">
          {icon}
        </div>
      </div>
      ...
      {trend !== undefined ? (
        <div className="mt-3.5 flex items-center gap-1.5 text-xs">
          ...
          <span className="text-zinc-400 dark:text-zinc-500 text-[11px]">{trendLabel}</span>
        </div>
      ) : subtitle ? (
